"""Tests for issue #146: Add POST /api/cases/{id}/summary endpoint with
caching (runs against UAT)"""
import os
import pytest
import httpx
...
def test_post_api_cases_summary_endpoint__endpoint_exists(client):
    ...
    assert r.status_code != 405, (
        "Endpoint should exist; 405 indicates missing route."
        f" Got {r.status_code}"
    )
...
def test_post_api_cases_summary_endpoint__422_pre_probe_stage(client):
    # AC5: Returns 422 Unprocessable Entity if the case has not yet reached
    # the probe stage
    pytest.skip(
        "manual — requires test case fixture at pre-probe stage"
        " (sharpened/bake_off/gather/weigh)"
    )


def test_post_api_cases_summary_endpoint__generates_summary_at_probe(client):
    # AC2: On success, a summary is generated and stored on case.summary,
    # then returned
    # AC8: The response body includes the summary text and indicates freshly
    # generated vs cached
    pytest.skip(
        "manual — requires test case fixture at probe stage in UAT database"
    )


def test_post_api_cases_summary_endpoint__caching_behavior(client):
    # AC3: If case.summary already exists and ?force=true is NOT passed,
    # cached value returned
    # AC8: The response indicates whether it was freshly generated or served
    # from cache
    pytest.skip(
        "manual — requires test case fixture with existing summary"
        " in UAT database"
    )


def test_post_api_cases_summary_endpoint__force_regenerate(client):
    # AC4: If ?force=true is passed, the summary is regenerated and
    # case.summary is overwritten
    pytest.skip(
        "manual — requires test case fixture with existing summary"
        " in UAT database"
    )
...
def test_post_api_cases_summary_endpoint__verdict_not_required(client):
    ...
    pytest.skip(
        "manual — requires test case fixture at probe stage without verdict"
        " in UAT database"
    )
