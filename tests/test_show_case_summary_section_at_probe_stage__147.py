"""Tests for issue #147: Show Case Summary section at probe stage (runs against UAT)"""
import os
import pytest
import pathlib
...
def test_case_summary_section_component_exists():
    # AC1: A Case Summary section is rendered inside CaseDetailScreen whenever case.stage === 'probe' (or any later stage).
...
def test_case_summary_section_rendered_at_probe_stage_or_later():
    ...
    assert "stage >= 4" in combined_js, \
        "CaseDetailScreen must render CaseSummarySection when stage >= 4 (probe stage)"


def test_case_summary_visible_without_verdict():
    # AC2: The section is visible even when no verdict exists (case.verdict is null or undefined)
    combined_js = _read_combined_js()
    # Find CaseSummarySection definition and verify it doesn't require verdict prop
    case_summary_start = combined_js.find("function CaseSummarySection")
    assert case_summary_start != -1, "CaseSummarySection must be defined as a function"
    case_summary_section = combined_js[case_summary_start:case_summary_start+5000]

    # Verify the component accepts hasVerdict prop but doesn't require it for rendering
    assert "hasVerdict" in case_summary_section, \
        "CaseSummarySection should accept hasVerdict prop"
    # The component should render regardless of verdict state (verified by checking it's inside the component)
...
def test_case_summary_not_rendered_before_probe_stage():
    # AC3: The section is NOT rendered when the case stage is earlier than probe
    combined_js = _read_combined_js()
    ...
    assert summary_usage != -1, "CaseSummarySection must be used in CaseDetailScreen"

    # Verify it's conditionally rendered with stage >= 4 check
    summary_context = case_detail_block[max(0, summary_usage-200):summary_usage+200]
    assert "stage >= 4" in summary_context or "{stage >= 4" in summary_context, \
        "CaseSummarySection should only render when stage >= 4 (not at sharpened/bake_off/gather/weigh)"
...
def test_case_summary_section_uses_design_tokens():
    ...
    combined_js = _read_combined_js()
    case_summary_start = combined_js.find("function CaseSummarySection")
    case_summary_section = combined_js[case_summary_start:case_summary_start+3000]
    ...
    assert "var(--text" in case_summary_section, \
        "CaseSummarySection should use text tokens (--text-*) for colors and sizing"


def test_case_summary_field_handling():
    # AC5: If the field is empty/absent, the section renders a legible placeholder rather than breaking
    combined_js = _read_combined_js()
    case_summary_start = combined_js.find("function CaseSummarySection")
    case_summary_section = combined_js[case_summary_start:case_summary_start+6000]

    # Verify conditional rendering of summary content vs placeholder
    assert "No summary" in case_summary_section, \
        "CaseSummarySection should render a placeholder when summary is empty"
    # Verify ternary or conditional check for summary presence
    assert "{summary ?" in case_summary_section, \
        "CaseSummarySection should conditionally render based on summary presence"


def test_case_summary_and_verdict_no_regression():
    # AC6: No regression — existing verdict section continues to render correctly when a verdict is present
    combined_js = _read_combined_js()
    case_detail_start = combined_js.find("function CaseDetailScreen")
    case_detail_block = combined_js[case_detail_start:]
    ...
    assert summary_pos != -1 and verdict_pos != -1, \
        "Both Case Summary and verdict sections should coexist in CaseDetailScreen"
...
def test_case_summary_sectionlabel_styling():
    ...
    combined_js = _read_combined_js()
    case_summary_start = combined_js.find("function CaseSummarySection")
    case_summary_section = combined_js[case_summary_start:case_summary_start+1000]

    # Verify SectionLabel component is used for the heading
    assert "SectionLabel" in case_summary_section, \
        "CaseSummarySection should use SectionLabel component for the heading"
    # Verify the label text
    assert "CASE SUMMARY" in case_summary_section, \
        "CaseSummarySection should have 'CASE SUMMARY' as the section heading"
