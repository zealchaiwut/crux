...
def test_support_status_has_server_default_unverified():
    ...
    from app.models import Source
    col = Source.__table__.c["support_status"]
    # Either server_default or column-level default should be 'unverified'
    has_server_default = (
...
def test_migration_file_handles_support_status_in_upgrade():
    """AC3: upgrade() must handle support_status (via add_column or alter_column/batch)."""
    f = _find_migration_file()
    assert f is not None, "Migration file missing"
    text = f.read_text()
    handles_support_status = (
        "support_status" in text
        and (
            "add_column" in text
            or "alter_column" in text
            or "batch_alter_table" in text
        )
    )
    assert handles_support_status, (
        "upgrade() must handle support_status via add_column, alter_column, or batch_alter_table"
    )


def test_migration_file_adds_support_rationale_in_upgrade():
    """AC3: upgrade() must call add_column for support_rationale."""
    f = _find_migration_file()
    assert f is not None, "Migration file missing"
    text = f.read_text()
    assert "add_column" in text and "support_rationale" in text, (
        "upgrade() must call op.add_column for support_rationale"
    )


def test_migration_file_drops_support_rationale_in_downgrade():
    """AC3: downgrade() must call drop_column to remove support_rationale."""
    f = _find_migration_file()
    assert f is not None, "Migration file missing"
    text = f.read_text()
    assert "drop_column" in text and "support_rationale" in text, (
        "downgrade() must call op.drop_column for support_rationale"
    )
...
def test_downgrade_removes_support_rationale_column(sqlite_migration_db):
    """AC5: After downgrade -1, support_rationale is removed and changes made by this
    migration are reversed cleanly.

    Note: support_status is not removed on downgrade because it was added by an earlier
    migration (j0k1l2m3n4o5). This migration only modifies its enum values and nullability;
    downgrade reverts those changes while leaving support_status in place.
    """
    ...
    assert "support_status" in columns, (
        f"support_status should still exist after downgrade -1 (it was added by an earlier migration); "
        f"remaining columns: {columns}"
    )
