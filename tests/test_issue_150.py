...
AC coverage:
  ...
  AC3 – POST /api/cases/{id}/summary returns HTTP 422 when stage is before
         probe.
...
_FAKE_SUMMARY = {
    ...
    "option_ranking": (
        "Option A: UX friction. Option B: page speed regression."
    ),
    "recommended_plan": (
        "Run an A/B test reverting the checkout flow."
    ),
    "probe_plan": (
        "Measurement probe tracking cart-to-purchase conversion rate."
    ),
}
...
def test_summary_endpoint_generates_on_first_call(api_client, db_session):
    """AC1: POST /api/cases/{id}/summary calls generate_summary and returns
    the result."""
    ...
    assert data.get("cached") is False, (
        "First call must return cached: False when no prior summary "
        "exists (AC1)"
    )
...
def test_summary_endpoint_caches_on_second_call(api_client, db_session):
    """AC1: Second POST without ?force=true returns cached:True with same
    summary."""
    c = _seed_case(db_session, stage="probe")
    ...
    assert mock_gen.call_count == 1, (
        "generate_summary must be called only once across two calls "
        "when force is not set (AC1)"
    )


def test_summary_stored_in_db_after_first_call(api_client, db_session):
    """AC1: After first call, the summary is persisted to case.summary in
    the DB."""

    c = _seed_case(db_session, stage="probe")
    assert c.summary is None, (
        "Summary must start as None before generation (AC1 precondition)"
    )
    ...
    assert c.summary is not None, (
        "Summary must be persisted to case.summary after first call (AC1)"
    )
...
def test_force_regenerates_when_cache_exists(api_client, db_session):
    """AC2: POST /api/cases/{id}/summary?force=true regenerates the
    summary."""
    ...
    with patch(
        ...
        r = api_client.post(
            f"/api/cases/{c.id}/summary", params={"force": "true"})
    ...
    data = r.json()
    assert data.get("cached") is False, (
        "?force=true must return cached: False even when a prior "
        "summary exists (AC2)"
    )
...
def test_force_updates_cached_summary_in_db(api_client, db_session):
    ...
    stored = json.loads(c.summary)
    assert (
        stored["problem_statement"] == _FAKE_SUMMARY_2["problem_statement"]
    ), (
        "DB summary must be updated to the force-regenerated value (AC2)"
    )


def test_without_force_skips_generation_when_cache_exists(
    api_client, db_session
):
    """AC2: Without ?force=true, generate_summary is NOT called when a cache
    exists."""
    c = _seed_case(db_session, stage="probe", summary=_FAKE_SUMMARY_JSON)
    ...
    assert r.json()["cached"] is True, (
        "Call without force must return cached: True when summary "
        "exists (AC2)"
    )
    assert mock_gen.call_count == 0, (
        "generate_summary must NOT be called when cache exists and "
        "force is not set (AC2)"
    )
...
@pytest.mark.parametrize(
    "pre_probe_stage",
    ["sharpened", "bake_off", "gather", "weigh"],
)
def test_summary_returns_422_before_probe_stage(
    api_client, db_session, pre_probe_stage
):
    """AC3: POST /api/cases/{id}/summary returns 422 for stages before
    probe."""
    ...
    assert r.status_code == 422, (
        f"Expected 422 for stage='{pre_probe_stage}', "
        f"got {r.status_code} (AC3)"
    )
    detail = r.json().get("detail", "")
    assert pre_probe_stage in detail or "probe" in detail.lower(), (
        "Error detail must mention the stage or 'probe' requirement "
        f"(AC3): {detail!r}"
    )


def test_summary_succeeds_at_probe_stage(api_client, db_session):
    """AC3 (boundary): POST /api/cases/{id}/summary succeeds at
    stage='probe'."""
    c = _seed_case(db_session, stage="probe")
...
def test_summary_succeeds_at_verdict_stage(api_client, db_session):
    ...
    assert r.status_code == 200, (
        "Summary must succeed at stage='verdict' "
        f"(AC3 boundary): {r.text}"
    )
...
def test_case_summary_section_has_placeholder_for_no_summary():
    """AC4: CaseSummarySection must render a placeholder when summary is
    null/undefined."""
    combined = _read_combined_js()
    block = _case_summary_section_block(combined)
    assert "No summary generated yet" in block, (
        "CaseSummarySection must include a 'No summary generated yet' "
        "placeholder for the pre-summary state (AC4)"
    )


def test_case_summary_section_renders_content_when_summary_present():
    """AC4: CaseSummarySection source must render summary fields when summary
    is present."""
    combined = _read_combined_js()
    block = _case_summary_section_block(combined)
...
def test_api_returns_null_summary_when_not_generated(api_client, db_session):
    """AC4: GET /api/cases/{id} returns summary: null when no summary has
    been generated."""
    ...
    assert data.get("summary") is None, (
        "API must return summary: null when no summary has been "
        "generated yet (AC4)"
    )


def test_case_summary_section_visible_without_verdict():
    """AC4: CaseSummarySection must render at stage >= 4 regardless of
    verdict presence."""
    combined = _read_combined_js()
    ...
    assert summary_pos != -1, (
        "CaseSummarySection must be used in CaseDetailScreen (AC4)"
    )
    ...
    assert "stage >= 4" in gate_ctx, (
        "CaseSummarySection must be gated on stage >= 4 (not verdict) "
        "so it renders in the pre-verdict state (AC4)"
    )
    # verdict_log must NOT be the direct gate for summary
    assert "!!caseData.verdict_log" not in gate_ctx, (
        "CaseSummarySection must NOT be gated on verdict_log — "
        "it renders pre-verdict (AC4)"
    )
...
def test_action_plan_gated_on_verdict_log_in_js():
    """AC5: The ACTION PLAN section in CaseDetailScreen is gated on
    verdict_log."""
    combined = _read_combined_js()
    block = _case_detail_block(combined)
    action_plan_idx = block.find("<SectionLabel>ACTION PLAN")
    assert action_plan_idx != -1, (
        "ACTION PLAN SectionLabel must exist in CaseDetailScreen (AC5)"
    )
...
def test_api_no_verdict_log_when_no_verdict(api_client, db_session):
    """AC5: GET /api/cases/{id} returns verdict_log: null when no verdict
    logged."""
...
def test_action_plan_not_gated_only_on_stage_without_verdict():
    """AC5: The ACTION PLAN gate must require BOTH stage >= 4 AND
    verdict_log."""
    combined = _read_combined_js()
    block = _case_detail_block(combined)
    action_plan_idx = block.find("<SectionLabel>ACTION PLAN")
    ...
    assert "stage >= 4" in preceding and "verdict_log" in preceding, (
        "ACTION PLAN gate must check BOTH stage >= 4 AND verdict_log "
        "so it remains locked pre-verdict even when at probe stage (AC5)"
    )
...
def test_action_plan_renders_verdict_details_in_js():
    """AC6: The ACTION PLAN section renders verdict outcome and notes from
    verdict_log."""
    combined = _read_combined_js()
    block = _case_detail_block(combined)
...
def test_api_verdict_log_present_after_verdict(api_client, db_session):
    """AC6: GET /api/cases/{id} returns a non-null verdict_log after verdict
    is logged."""
    c = _seed_case(db_session, stage="verdict", with_verdict=True)
    r = api_client.get(f"/api/cases/{c.id}")
    assert r.status_code == 200
...
def test_action_plan_unlocked_state_includes_leading_plan():
    """AC6: The unlocked ACTION PLAN section must show the LEADING PLAN
    sub-section."""
    combined = _read_combined_js()
    assert "LEADING PLAN" in combined, (
        "The unlocked ACTION PLAN section must include a LEADING PLAN "
        "sub-section (AC6)"
    )
